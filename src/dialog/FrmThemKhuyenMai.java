...
public class FrmThemKhuyenMai extends JFrame {
        ...
        try {
            ...
            if (!laTangMon) {
                try {
                    ...
                    if (giaTri <= 0) {
                        JOptionPane.showMessageDialog(this, "Giá trị khuyến mãi phải > 0!");
                        return;
                    }

                    if (txtDonHangTu.getText().trim().isEmpty()) {
                        JOptionPane.showMessageDialog(this, "Đơn hàng từ không được để trống!");
                        return;
                    }
                    donHangTu = Double.parseDouble(txtDonHangTu.getText().trim());
                    if (donHangTu < 0) {
                        JOptionPane.showMessageDialog(this, "Đơn hàng từ phải ≥ 0!");
                        return;
                    }
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(this, "Giá trị, đơn hàng từ và giảm tối đa phải là số!");
                    return;
                }
            } else {
                //KM TẶNG MÓN
    ...
    private void lamMoi() {
        txtMaKM.setText(kmDAO.taoMaKhuyenMaiMoi());
        txtTenKM.setText("");
        txtGiaTri.setText("");
